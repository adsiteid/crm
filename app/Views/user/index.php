...
<?php foreach ($users->user(user()->id)->getresultArray() as $id_user) : ?>
    ...
    <?php foreach ($users->list($id_user['groups'])->getresultArray() as $row) : ?>
                ...
                <div class="d-flex align-items-center justify-content-between  pb-3 row ">
                    <div class="col-lg-5 col-md-3 col-sm-3 col-12 d-lg-block d-md-block d-sm-block d-none">
                        ...
                        <?php if (in_groups('users')) : ?>
                            <div type="button" class=" btn btn-sm btn-primary shadow-sm mr-1 " style="font-size:12px;"> <?php foreach ($group->detail($row['groups'])->getResultArray() as $gr) : ?> <?= $gr['group_name']; ?><?php endforeach; ?> </div>
                        <?php endif; ?>
                    </div>
